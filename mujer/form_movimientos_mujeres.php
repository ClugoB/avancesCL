    ...
    <div class="logo-name"><span>MinMujer</span></div>
        ...
        <?php if (in_array('ver_panel', $permisosPermitidos)): ?>
            ...
            <?php if (in_array('ver_usuarios', $permisosPermitidos)): ?>
                <li><a href="form_movimientos_mujeres.php"><i class='bx bx-file'></i>INGRESO DE MOV</a></li>
            <?php endif; ?>
            <?php if (in_array('ver_usuarios', $permisosPermitidos)): ?>
                <li><a href="aceptar_ayudantes.php"><i class='bx bx-check'></i>ACEPTAR AYUDANTES</a></li>
            <?php endif; ?>
            <?php if (in_array('ver_panel', $permisosPermitidos)): ?>
...
<!-- MAIN -->
        <main>
            <div class="header">
                <div class="left">
                    <h1>FORMULARIO DE AYUDANTES PARA INGRESAR AL MOVIMIENTO</h1>
                </div>
            </div>
            <!-- FORMULARIO -->
<section class="formulario_mujer">
  <form action="conexion_mov_ayudantes.php" method="post">
...
<div class="caja_form"> 
    <div class="input_form">
        ...
        <select id="tipos_movimientos" name="tipos_movimientos" onchange="cargarMovimientos()">
            <option value="" disabled selected>Seleccione un tipo de movimiento</option>
        </select>
    </div>
    <!-- MOVIMIENTOS DISPONIBLES -->
    <div class="input_form">
        <label for="nombre_movimiento">Movimientos:</label><br>
        <select id="nombre_movimiento" name="nombre_movimiento">
...
<div class="caja_form">
    <div class="input_form">
        ...
        <input type="text" id="nombre_apellido" name="nombre_apellido" required placeholder="Ingrese el cargo">
    </div>
    <!-- CARGO DEL AYUDANTE -->
    <div class="input_form">
...
<div class="caja_form">
    <div class="input_form">
        <label for="embarazada">¿Se encuentra embarazada?:</label><br>
        <select id="embarazada" name="embarazada" onchange="toggleTiempoEncinta()" required>
    ...
    <!-- TIEMPO ENCINTA -->
    <div class="input_form">
        <label for="tiempo_embarazada">¿Cuánto tiempo lleva?:</label><br>
        <select id="tiempo_embarazada" name="tiempo_embarazada" disabled>
...
<div class="caja_form">
    <div class="input_form">
        ...
        <select id="discapacidad" name="discapacidad" onchange="toggleTipoDiscapacidad()" required>
        <option value="" disabled selected>Seleccione una opción</option>
            <option value="Si">Sí</option>
            <option value="No">No</option>
        </select>
...
<script>
// Función para cargar los tipos de movimiento en el primer select
function cargarTiposMovimiento() {
    $.ajax({
        type: "GET",
        url: "conexion_movimientos_mujeres.php", 
        dataType: "json",
        success: function(data) {
            $.each(data, function(index, value) {
                $("#tipos_movimientos").append("<option value='" + value.tipos_movimientos + "'>" + value.tipos_movimientos + "</option>");
            });
        },
...
// Función para cargar los movimientos en el segundo select en función del tipo de movimiento seleccionado
function cargarMovimientos() {
    var tipoMovimiento = $("#tipos_movimientos").val();
    $.ajax({
        type: "GET",
        url: "conexion_movimientos_mujeres.php",
        data: { tipos_movimientos: tipoMovimiento },
        dataType: "json",
        success: function(data) {
            $("#nombre_movimiento").empty();
            // Mantener la opción "Seleccione un movimiento"
            $("#nombre_movimiento").append("<option value='' disabled selected>Seleccione un movimiento</option>");
            $.each(data, function(index, value) {
                $("#nombre_movimiento").append("<option value='" + value.nombre_movimiento + "'>" + value.nombre_movimiento + "</option>");
            });
        },
        error: function(xhr, status, error) {
...
// Llamar a la función para cargar los tipos de movimiento al cargar la página
...
<script>
    function toggleTiempoEncinta() {
        var encintaSelect = document.getElementById("embarazada");
        var tiempoSelect = document.getElementById("tiempo_embarazada");
